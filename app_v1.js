...
/* ── Language ─────────────────────────────────────────────── */
let LANG = localStorage.getItem('bustanim_lang') || 'he';
const TR = {
  he:{
    loading:'טוען נתונים...',
    ready:'מוכן',
...
function featLabel(feat,lk){
  ...
  const tf=meta.typeField; return(tf&&p[tf])||meta.name_he;
}
...
async function showModal(feat,lk){
  ...
  document.getElementById('objModalContent').innerHTML=`
    <div style="display:flex;align-items:center;gap:12px;margin-bottom:16px">
      ...
      <div>
        <div style="font-size:19px;font-weight:700;color:#1e40af">${typeVal||meta.name_he} #${p.OBJECTID}</div>
        <div style="font-size:12px;color:#475569">${meta.name_he}</div>
      </div>
    </div>
    <div>${rows.join('')}</div>`;
...
function updateOverview(){
  ...
  document.getElementById('kAvgH').textContent=fmt(mean(heights))+' ס"מ';
  document.getElementById('kAvgG').textContent=fmt(mean(girths))+' ס"מ';
...
function refreshDistrictDetail(){
  ...
  Plotly.newPlot('chartDistrictSpeciesBar',[{type:'bar',x:top15.map(s=>s.count),y:top15.map(s=>s.name),orientation:'h',marker:{color:top15.map(s=>s.lkColor)},text:top15.map(s=>s.count),textposition:'outside'}],layout('הרכב מינים — '+nafa+(groupFilter?' ('+LAYER_META[groupFilter].name_he+')':''),'כמות',''),cfgPlot());
...
function showDistrictSpeciesDetail(nafa,spec){
  ...
  document.getElementById('districtSpeciesDetailTitle').textContent=spec.name+' — '+nafa+' ('+spec.count+' אובייקטים)';
...
function showSelectionDetail(feat,lk){
  ...
  document.getElementById('selectionDetail').innerHTML=`
    <div class="card" style="margin-top:10px">
      <h5>${typeVal||meta.name_he} #${p.OBJECTID}</h5>
...
function renderAnalyticsCharts(){
  ...
  if(stEl){
    const parts=[];
    if(grp)parts.push('תחום: '+LAYER_META[grp].name_he);
    if(spec)parts.push('מין: '+spec);
    if(nafaSel.length)parts.push('נפות: '+nafaSel.join(', '));
    stEl.textContent=total.toLocaleString('he-IL')+' אובייקטים'+(parts.length?' | '+parts.join(' | '):'');
  }
  const pc=cfgPlot();
  /* 1 – Type distribution */
  ...
  Plotly.newPlot('chartTypeDist',[{type:'bar',x:tKeys,y:tKeys.map(k=>td[k]),marker:{color:tKeys.map((_,i)=>TYPE_COLORS[i%TYPE_COLORS.length])},text:tKeys.map(k=>td[k]),textposition:'outside'}],layout('התפלגות סוגים — '+(grp?LAYER_META[grp].name_he:'כל התחומים'),'סוג','כמות'),pc);
  /* 2 – Height histogram */
...
function updateAdvanced(){
  ...
  Plotly.newPlot('chartTypeByBustan',lks3.map((lk,i)=>{
    const dist=typeDist(lk);
    return{type:'bar',name:LAYER_META[lk].name_he,x:allTypesMerged,y:allTypesMerged.map(tv=>dist[tv]||0),marker:{color:LAYER_META[lk].color}};
...
function applyLang(){
  ...
  document.getElementById('btnToggleLang').textContent=LANG==='he'?'English':'עברית';
...
async function init(){
  ...
  document.getElementById('btnToggleLang').addEventListener('click',()=>{
    LANG=LANG==='he'?'en':'he';localStorage.setItem('bustanim_lang',LANG);
